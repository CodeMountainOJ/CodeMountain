...
pub async fn registration_handler(
    conn_pool: Data<Pool>,
    req: Json<payload::RegisterRequest>,
    ...
    let is_unique = is_unique(req_firstname, req_username, req_email, conn_pool.as_ref())?;
    ...
    let salted_password =
        hash(&req.password, 7).map_err(|_| errors::Errors::InternalServerError)?;
    ...
    .map_or_else(Err, |_| {
        Ok(actix_json(payload::ReturnStatus { success: true }))
    })
}
